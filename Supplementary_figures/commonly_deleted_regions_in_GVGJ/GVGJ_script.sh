...
paste *.1kbcov |grep -v "AA" |grep -v "MT" |grep -v "KZ" | grep -v "W"|awk -F '\t' '{if($4<5 && $8<5 && $12<5 && $16<5 && $20<5)print $0}' |cut -f1,2,3 |bedtools merge -i stdin -d 10 -c 3 -o collapse |cut -f1,2,3  >GVGJ.bed
##replacing the tab with underscore
sed 's/\t/_/g' GVGJ.bed |grep -v "_0_" > GVGJ_del.list
##make 2kb flank bed file  both side from actual position of deletion
for j in `cat GVGJ_del.list`
do 
k=`echo $j |cut -f1 -d "_"`
l=`echo $j |cut -f2 -d "_"`
m=`echo $j |cut -f3 -d "_"`
